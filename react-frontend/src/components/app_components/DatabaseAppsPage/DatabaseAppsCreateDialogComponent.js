...
const DatabaseAppsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,environment: _entity?.environment?._id,username: _entity?.username,password: _entity?.password,port: _entity?.port,url: _entity?.url,database: _entity?.database,
        ...
        const result = await client.service("databaseApps").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Database Apps updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Database Apps" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Database Apps" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="databaseApps-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputText id="database" className="w-full mb-3 p-inputtext-sm" value={_entity?.database} onChange={(e) => setValByKey("database", e.target.value)}  />
            </span>
            <small className="p-error">
...
export default connect(mapState, mapDispatch)(DatabaseAppsCreateDialogComponent);
