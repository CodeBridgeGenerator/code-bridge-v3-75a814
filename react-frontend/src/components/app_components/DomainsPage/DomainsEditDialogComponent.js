...
const DomainsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            name: _entity?.name,
environment: _entity?.environment?._id,
domain: _entity?.domain,
appName: _entity?.appName,
appDir: _entity?.appDir,
appProjectName: _entity?.appProjectName,
appObjectName: _entity?.appObjectName,
appPortNumber: _entity?.appPortNumber,
isDefault: _entity?.isDefault,
        ...
        await client.service("domains").patch(_entity._id, _data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info domains updated successfully" });
    ...
    return (
        <Dialog header="Edit Domains" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="domains-edit-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="appPortNumber" className="w-full mb-3 p-inputtext-sm" value={_entity?.appPortNumber} onChange={(e) => setValByKey("appPortNumber", e.value)}  useGrouping={false}/>
            </span>
            <small className="p-error">
