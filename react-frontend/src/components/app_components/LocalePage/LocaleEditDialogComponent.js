...
const LocaleCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            localeName: _entity?.localeName,
dateFormat: _entity?.dateFormat,
timeFormat: _entity?.timeFormat,
timeZone: _entity?.timeZone,
currencyFormat: _entity?.currencyFormat,
currencyIndex: _entity?.currencyIndex,
currencySymbol: _entity?.currencySymbol,
country: _entity?.country,
language: _entity?.language,
languages: _entity?.languages,
intlDialing: _entity?.intlDialing,
isDefault: _entity?.isDefault,
        ...
        const result = await client.service("locale").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info locale updated successfully" });
    ...
    return (
        <Dialog header="Edit Locale" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="locale-edit-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="intlDialing" className="w-full mb-3 p-inputtext-sm" value={_entity?.intlDialing} onChange={(e) => setValByKey("intlDialing", e.value)}  useGrouping={false}/>
            </span>
            <small className="p-error">
