...
const GitRepositoriesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name,environment: _entity?.environment?._id,isDefault: _entity?.isDefault || false,
        ...
        const result = await client.service("gitRepositories").create(_data);
        const eagerResult = await client
            .service("gitRepositories")
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Git Repositories updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Git Repositories" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Git Repositories" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="gitRepositories-create-dialog-component">
...
export default connect(mapState, mapDispatch)(GitRepositoriesCreateDialogComponent);
